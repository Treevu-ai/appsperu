...
Servidor MCP que expone las 9 APIs de este repo (`apps/*/api`) como tools de solo lectura para
...
Las 9 APIs deben estar corriendo (ver [`docs/ESTADO.md`](../docs/ESTADO.md) — `docker compose up
...
57 tools, uno por endpoint `GET /api/*` real de las 9 apps (`src/catalog.ts` es la fuente de
...
- **Transporte**: solo stdio (uso local, agente y las 9 APIs en la misma máquina). Streamable
  HTTP para exponerlo remoto es un paso posterior, no implementado.
- **Sin autenticación**: igual que las 9 APIs que agrega (`helmet` + `cors` + rate limit, sin auth
...
- Validado manualmente: registro de tools, llamada con query params reales, manejo de error de
  ...
  (`src/__tests__/catalog.test.ts`). No hay suite contra las 9 APIs reales corriendo en CI.
