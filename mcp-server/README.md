...
Servidor MCP que expone las 13 APIs de este repo (`apps/*/api`) como tools de solo lectura para
...
Las 13 APIs deben estar corriendo (ver [`docs/ESTADO.md`](../docs/ESTADO.md) — `docker compose up
...
`INVERSION_PRIVADA_API_URL`.

## Catálogo de tools

82 tools (13 apps), uno por endpoint `GET /api/*` real de las 13 apps (`src/catalog.ts` es la fuente de
...
- **Transporte**: solo stdio (uso local, agente y las 13 APIs en la misma máquina). Streamable
  HTTP para exponerlo remoto es un paso posterior, no implementado.
- **Sin autenticación**: igual que las 13 APIs que agrega (`helmet` + `cors` + rate limit, sin auth
...
- Validado manualmente: registro de tools, llamada con query params reales, manejo de error de
  ...
  (`src/__tests__/catalog.test.ts`). No hay suite contra las 13 APIs reales corriendo en CI.
