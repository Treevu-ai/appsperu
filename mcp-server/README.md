...
Servidor MCP que expone las 8 APIs de este repo (`apps/*/api`) como tools de solo lectura para
...
Las 8 APIs deben estar corriendo (ver [`docs/ESTADO.md`](../docs/ESTADO.md) — `docker compose up
...
`RADAR_INVERSIONES_API_URL`, `INFOBRAS_API_URL`, `CEPLAN_ESTRATEGICO_API_URL`,
`IDENTIDAD_FISCAL_API_URL`, `SALUD_INSTITUCIONAL_API_URL`, `PROVEEDORES_SANCIONADOS_API_URL`.

## Catálogo de tools

45 tools, uno por endpoint `GET /api/*` real de las 8 apps (`src/catalog.ts` es la fuente de
...
- **Transporte**: solo stdio (uso local, agente y las 8 APIs en la misma máquina). Streamable
  HTTP para exponerlo remoto es un paso posterior, no implementado.
- **Sin autenticación**: igual que las 8 APIs que agrega (`helmet` + `cors` + rate limit, sin auth
