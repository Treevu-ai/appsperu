...
const OcidParamSchema = z.object({
  ocid: z.string().min(1).startsWith("ocds-", "OCID inválido. Debe comenzar con 'ocds-'"),
});
 ...
 */
biddersRouter.get("/:ocid", asyncHandler(async (req, res) => {
  const parsedParams = OcidParamSchema.safeParse(req.params);
  if (!parsedParams.success) {
    res.status(400).json({ error: "OCID inválido. Debe comenzar con 'ocds-'" });
    return;
  }
  const { ocid } = parsedParams.data;
 ...
 */
biddersRouter.get("/provider/:provider_id", asyncHandler(async (req, res) => {
  const parsedParams = ProviderIdParamSchema.safeParse(req.params);
  if (!parsedParams.success) {
    res.status(400).json({ error: "provider_id inválido." });
    return;
  }
  const { provider_id } = parsedParams.data;
 ...
 */
biddersRouter.get("/analytics/competition", asyncHandler(async (_req, res) => {
  const result = await pool.query(
    `SELECT
      bidder_name,
      bidder_id,
 ...
 */
biddersRouter.get("/analytics/co-participation", asyncHandler(async (_req, res) => {
  const result = await pool.query(
