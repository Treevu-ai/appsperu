...
export const indicatorsExecutionEfficiencyRouter = Router();
...
const NACIONAL_RESTRICCION =
  "Execution Efficiency nacional CEPLAN = CUMP02 / CUMP03 por nivel de gobierno; no implica cobertura departamental.";

indicatorsExecutionEfficiencyRouter.get(
  "/",
  asyncHandler(async (req, res) => {
    const parsed = parseQuery(EfficiencyQuerySchema, req.query, res);
    if (!parsed) return;
    ...
    res.json({
      ...
      resultados: [...CROSSREFEABLE_NIVELES_GOBIERNO].map((nivel) => {
        ...
        return {
          ...
          cump02: item.cump02,
          cump03: item.cump03,
          executionEfficiency: item.executionEfficiency,
        };
      }),
    });
  })
