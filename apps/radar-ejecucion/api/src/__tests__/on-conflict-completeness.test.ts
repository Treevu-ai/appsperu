 ...
 *
 * Este test no ejecuta los connectors — parsea el SQL fuente. Para cada
 * bloque `INSERT INTO t (...) ... ON CONFLICT (...) DO UPDATE SET ...`
 * verifica que toda columna del INSERT (salvo la clave de conflicto)
 * aparezca en el SET. Columnas intencionalmente insert-only (ej. first_seen
 * en supplier_profiles) se excluyen explícitamente por archivo/tabla.
 */
...
describe("ON CONFLICT DO UPDATE SET completeness (regresión invierte-connector, 2026-09-05)", () => {
  it("bienes_muebles_baja: toda columna insertada se refresca en un re-ingest", () => {
    const source = readSource("../ingest/bienes-muebles-baja-connector.ts");
    const [block] = extractQueryBlocks(source).filter((b) => b.table === "bienes_muebles_baja");
    assertQueryBlockComplete(block);
  });

  it("airhsp_personal: toda columna insertada se refresca en un re-ingest", () => {
    const source = readSource("../ingest/airhsp-connector.ts");
    const [block] = extractQueryBlocks(source).filter((b) => b.table === "airhsp_personal");
    assertQueryBlockComplete(block);
  });

  it("tourism_hospitality_monthly: toda columna insertada se refresca en un re-ingest", () => {
    const source = readSource("../ingest/mincetur-hospedaje-connector.ts");
    const [block] = extractQueryBlocks(source).filter((b) => b.table === "tourism_hospitality_monthly");
    assertQueryBlockComplete(block);
  });
});
