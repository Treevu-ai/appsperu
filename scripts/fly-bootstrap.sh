...
#
# Variables:
#   FLY_APP_PREFIX   default rastro-api (único en Fly.io — "treevu-rastro-*" puede
#                    estar tomado por una cuenta anterior sin acceso; ver docs/API_PROXY_DEPLOY.md)
...
#   API_HOSTNAME     default api.rastro.fyi (dominio público final, ver fly-dns-api-rastro-fyi.sh)
#
# Uso:
...
API_HOSTNAME="${API_HOSTNAME:-api.rastro.fyi}"
...
ensure_fly_app() {
  ...
  echo "  FLY_APP_PREFIX=rastro-api-$(date +%s) bash scripts/fly-bootstrap.sh --skip-pg" >&2
  return 1
}
...
echo "  fly certs add ${API_HOSTNAME} -a ${GATEWAY_APP}"
echo "  fly certs show ${API_HOSTNAME} -a ${GATEWAY_APP}"
echo "  CLOUDFLARE_API_TOKEN=... bash scripts/fly-dns-api-rastro-fyi.sh"
echo "  (recién con el cert validado) activar proxy en Cloudflare + Access — ver docs/API_ACCESS_PROTECTION.md"
