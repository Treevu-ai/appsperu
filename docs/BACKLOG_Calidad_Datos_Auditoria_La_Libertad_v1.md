...
| DQ-17 | Crosswalk `entity_crosswalk` (infobras) vincula entidades de departamentos distintos; nuevas filas `distrito_sospechoso` fuera de Pataz tras CT-06 | `entity_crosswalk` nunca liga una entidad de un departamento a una obra de otro (mínimo: excluir o re-evaluar el match cruzado Chavimochic↔Chinecas); auditoría de `distrito_sospechoso` corre sobre las 25 regiones ingeridas por CT-06, no solo La Libertad | CT-06 (ya cerrado) | P1 | M | ⬜ Pendiente — ver hallazgo 2026-09-09 abajo |
