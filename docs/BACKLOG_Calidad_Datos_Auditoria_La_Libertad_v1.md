...
| DQ-17 | Crosswalk `entity_crosswalk` (infobras) vincula entidades de departamentos distintos; nuevas filas `distrito_sospechoso` fuera de Pataz tras CT-06 | `entity_crosswalk` nunca liga una entidad de un departamento a una obra de otro (mínimo: excluir o re-evaluar el match cruzado Chavimochic↔Chinecas); auditoría de `distrito_sospechoso` corre sobre las 25 regiones ingeridas por CT-06 (objetivo eventual — alcance activo acotado a LA LIBERTAD/AREQUIPA/LIMA desde 2026-09-09, ver `docs/ESTADO.md`), no solo La Libertad | CT-06 (ya cerrado) | P1 | M | 🟡 Parcial (2026-09-09) — fix del matcher + auditoría nacional hechos; caso Sarín (departamento completo errado) queda fuera de alcance, ver TICKETS |
| DQ-18 | `provincia`/`distrito` estructurados no confiables en OxI Ficha técnica/Por Priorizar (`inversion-privada`) | Filas OxI con `provincia` no-texto (ej. código numérico inestable) quedan marcadas, no citadas como confiables; evaluar fallback verificado desde `nombreProyecto` | — | P2 | S | ⬜ Pendiente — ver hallazgo 2026-09-10 en TICKETS |
| DQ-19 | 889 obras de Ica (Nazca/Chincha/Pisco/Palpa) mal etiquetadas como HUANCAVELICA en INFOBRAS — causa raíz del outlier de DQ-17 | Decisión de producto documentada: corregir `departamento` en post-proceso del ingest, o dejar solo `distrito_sospechoso` como señal; si se corrige, verificar que no rompe conteos de cobertura territorial ya cerrados (CT-06) | DQ-17 | P1 | M | ⬜ Pendiente — causa raíz investigada, sin decisión ni fix, ver TICKETS |
