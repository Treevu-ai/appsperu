...
- **Desambiguación territorial exigida (hallazgo real de CodeRabbit)**: dado que puede haber UGEL homónimas en distintos departamentos, la agregación por UGEL normalizada debe exigir también la misma DRE o departamento entre el padrón y SíseVe antes de sumar un caso a esa UGEL; alternativamente, si SíseVe no trae DRE/departamento con la granularidad necesaria, el endpoint debe rechazar explícitamente (documentarlo en la respuesta, no solo en logs) cualquier colisión detectada entre UGEL homónimas de distintos departamentos, sin introducir un crosswalk nuevo ni un matcher difuso — mismo alcance de normalización que el punto anterior (mayúsculas/tildes únicamente).
