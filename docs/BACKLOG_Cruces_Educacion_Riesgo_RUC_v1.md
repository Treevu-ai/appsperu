...
|---|---|---|---|---|---|---|---|
| EDU-01 | Cruce educación | `GET /api/crossref` en `violencia-escolar`: casos por UGEL × trayectoria SIAGIE agregada a UGEL vía el padrón. | Normaliza mayúsculas/tildes de UGEL explícitamente; expone tasa de violencia por matrícula, no solo conteo crudo; UGEL sin match en null, no 0; verificado en vivo contra La Libertad (15 UGEL). | Ninguna. | P0 | M | 0 |
...
| EDU-02 | Cruce educación | `GET /api/crossref` en `instituciones-educativas`: SIAGIE trayectoria por UBIGEO × `budget_execution` (`FUNCION = EDUCACIÓN`). | Valor real de `FUNCION` confirmado con `SELECT DISTINCT` documentado en el PR; declara cobertura territorial real de `budget_execution`; distrito sin match no aparece con ejecución cero. | Ninguna. | P1 | M | 1 |
...
| EDU-03 | Cruce educación | `GET /api/resumen` o `/crossref` en `violencia-escolar`: casos por departamento × ejecución presupuestal educativa. | Respuesta incluye campo explícito de advertencia sobre causalidad/acceso (no solo en docs); mismo criterio de `DISTINCT` que EDU-02 para `FUNCION`. | Ninguna. | P2 | S | 2 |
