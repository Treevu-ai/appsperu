...
| ADS-15 | **CERRADO** (contrato) | ~~Confirmar contrato completo de `api.congreso.gob.pe/spley-portal-service`~~ — confirmado 2026-09-21: `curl` puro sin sesión de navegador, `perParId` único campo requerido, catálogo real de periodos (`2021`, `2026`) y filtros descubiertos. Ver `docs/data-contracts/congreso-spley-portal-service.md`. Evaluación de votaciones/asistencia/comisiones bajo el mismo host queda pendiente, no bloqueante (ver el data contract). | Consulta real con `200` y datos, body exacto documentado ✅; contrato de `FiltroProyecLeyDto` documentado ✅. | Ninguna. | P1 | S | 1 |
