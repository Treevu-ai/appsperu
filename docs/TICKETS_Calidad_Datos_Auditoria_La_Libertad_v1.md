...
### DQ-19 · Bloque grande de obras de Ica (Nazca/Chincha/Pisco/Palpa) mal etiquetado como HUANCAVELICA en INFOBRAS

- **Historia:** Como consumidor de `infobras`/`salud-institucional`, quiero saber si el conteo de obras por departamento de HUANCAVELICA e ICA es confiable, dado que la auditoría nacional de DQ-17 encontró que HUANCAVELICA es un outlier extremo (15.3% `distrito_sospechoso` vs. 3.7% nacional).
- **Contexto verificado (2026-09-09, investigación directa del outlier de HUANCAVELICA):** desglosando `public_works` de HUANCAVELICA por `provincia`, 4 de las 11 provincias que aparecen bajo `departamento = 'HUANCAVELICA'` **no son provincias de Huancavelica** — son provincias reales de **ICA**: NAZCA (330 filas, 330 `distrito_sospechoso`), CHINCHA (324 filas, 307 sospechosas), PISCO (208 filas, 208 sospechosas), PALPA (27 filas, 27 sospechosas). Total: **889 obras** (63% de las 1,409 sospechosas de "Huancavelica") que en realidad pertenecen a ICA. Verificado con muestras de `nombre_obra` inequívocas: "...I E N 22395 MOISÉS REBATA DEL NIVEL PRIMARIO EL INGENIO NASCA", "CENTRAL EÓLICA PARQUE NAZCA 126 MW", "MUNICIPALIDAD DISTRITAL DE CHANGUILLO" (distrito real de la provincia de Nazca) — todas con `departamento = 'HUANCAVELICA'` en la fila.
  - **No es un bug de nuestro ingest**: `canonicalizarDepartamentoFuente()` (`infobras-connector.ts`) no tiene ningún alias que mapee Ica→Huancavelica (solo existe el alias de Callao, CT-06). El valor llega así directo de la columna 29 del XLSX nacional de INFOBRAS — es un error de la fuente misma, a diferencia del caso Callao que sí era un alias de nombre normalizable.
  - **El mecanismo de detección ya funciona como se diseñó**: `distritoEsSospechoso()` (DQ-14) marca correctamente el 100% de NAZCA/PISCO/PALPA y el 95% de CHINCHA como `distrito_sospechoso = true`, porque esos distritos genuinamente no pertenecen al universo de Huancavelica en el catálogo territorial. Esto no es una detección fallida — es la señal funcionando; el problema es la escala (889 filas de un solo lote, no casos aislados como Pataz o Chavimochic).
  - Restante de HUANCAVELICA (provincias reales: TAYACAJA, HUANCAVELICA, ACOBAMBA, ANGARAES, CHURCAMPA, HUAYTARA, CASTROVIRREYNA) también tiene una tasa interna alta en la provincia HUANCAVELICA misma (409/2,095 = 19.5% sospechosas) — sin investigar todavía, podría ser un problema distinto y menor.
- **Criterios de aceptación:**
  - Decidir si corregir el `departamento` de estas 889 filas (reclasificarlas a ICA) en un paso de post-procesamiento del ingest, o dejarlas como están y depender de `distrito_sospechoso` para que los consumidores las excluyan — **no se decidió en esta sesión**, es una decisión de producto (¿corregimos el dato de la fuente o solo lo señalizamos?), no solo técnica. **Importante (hallazgo de CodeRabbit en PR #144):** la opción "depender de `distrito_sospechoso`" NO es un fallback completo — CHINCHA solo tiene 307/324 filas marcadas (95%, ver arriba), así que 17 de las 889 filas de Ica quedarían sin marcar y sin excluir bajo esa opción. Si se opta por señalizar en vez de corregir, hace falta cerrar ese 5% (ej. una regla adicional o aceptar explícitamente el gap residual), no asumir cobertura 100%.
  - Si se decide corregir: la heurística es simple y de bajo riesgo porque el patrón es 100% consistente — cuando `provincia` pertenece al catálogo real de ICA, `departamento` debería ser `ICA`, sin importar lo que diga la columna fuente.
  - Investigar aparte la tasa interna alta (19.5%) de la propia provincia HUANCAVELICA.
