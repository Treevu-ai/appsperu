...
- Se determina explícitamente una de tres conclusiones, documentada con su evidencia: (a) automatizable vía `fetch()` con las cabeceras correctas, (b) manual-asistida (navegador real para descargar, parseo automático después, como `riesgo-fiscal-isds`), (c) no automatizable de forma responsable en este momento.
