...
| **Rendición de cuentas / cierre presupuestal** | `informes-control` (`informes-control-connector.ts`) | Metadatos de informes de auditoría (entidad, ubicación, fechas, sector, tipo de servicio, indicador booleano de responsabilidad) vía reverse engineering del buscador de la Contraloría (`BusquedaInformesCGR.ashx`, mismo método que OECE). El contenido sustantivo de cada informe sigue en PDF (no se descarga ni procesa). | **Hueco cerrado 2026-09-05** para el metadato estructurado — construido, probado, verificado en vivo (363,971 informes históricos confirmados, ingesta por año). El contenido de los PDF individuales sigue fuera de alcance. |
...
| `proveedores-sancionados` | **Sí, real**: `razon_social` en `inhabilitaciones`/`multas` es literalmente el nombre de la persona cuando el sancionado tiene RUC-10 — no hay separación entre "nombre de empresa" y "nombre de persona natural" en el esquema. **Ampliado 2026-09-06**: `GET /api/crossref/personas-sancionadas` extrae el DNI incrustado en el RUC-10 (columna generada, no dato nuevo) y lo cruza contra `numero_documento` de `supplier_conformacion` (compras-publicas) para detectar personas sancionadas que son socio/representante de una empresa activa — mismo patrón de enriquecimiento entre fuentes que la propia tabla advertía evitar en la fila anterior de este documento, con la misma mitigación ya usada en conformación societaria: el DNI nunca se expone completo (`dniEnmascarado`, últimos 3 dígitos), solo se usa como clave de cruce interno. | Ley 27806 — el RNP/Tribunal de Contrataciones publica estas sanciones precisamente para que sean conocidas (es un registro de inhabilitación con efecto legal erga omnes); republicar un dato que la propia entidad sancionadora hace público con ese fin tiene base sólida. El cruce por DNI no trata un dato nuevo (el DNI ya vivía en el RUC-10 ingerido desde el inicio) — solo lo hace explícito para conectar dos registros que ya eran cada uno independientemente público. | **Medio** (sin cambio — el enriquecimiento nuevo usa la misma mitigación de enmascarado que ya bajó el riesgo de conformación societaria, no introduce una fuente de dato personal nueva) | Declarar explícitamente en una política de privacidad que este dataset republica sanciones ya públicas del RNP con fin de interés público (control ciudadano del gasto/conflicto de interés), y mantener el enmascarado de DNI en cualquier cruce adicional que se agregue — nunca exponerlo completo, sin importar cuántas fuentes se sumen al cruce. |
...
| `informes-control` (Contraloría, **añadido 2026-09-05**) | **No, por diseño verificado con test**: la fuente real mezcla campos de entidad con `Funcionarios`/`Responsabilidad` (persona natural) en la misma respuesta, pero el conector nunca lee esas claves del objeto crudo — solo conserva `es_con_responsabilidad` como booleano. Mismo método de descubrimiento (reverse engineering de un endpoint no documentado) que el conector de conformación societaria de OECE, pero con exclusión activa de PII en vez de captarla y enmascararla después. | Ley 27806 (dato de entidad ya público en el buscador oficial); Ley 29733 no aplica — no hay dato personal tratado. | **Bajo** | Ninguna adicional sobre el dato — el riesgo residual de "endpoint no documentado, sin autorización explícita" es el mismo tipo que OECE (ver fila de conformación societaria), pero sin el agravante de PII: si Contraloría lo objeta, se desactiva el conector sin haber tratado nunca un dato personal. |
