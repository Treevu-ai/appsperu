...
|---|---|---|---|---|---|---|---|
| LEG-01 | **IMPLEMENTADO** (Ingesta) | Conector `legislativo-congreso` — ingesta de proyectos de ley por periodo parlamentario. Verificado en vivo 2026-09-21: 14,868/14,868 filas, 0 rechazadas, periodos descubiertos dinámicamente vía `GET /periodo-parlamentario`. | Usa el contrato exacto confirmado por ADS-15 ✅; clave de upsert `perParId`+`pleyNum` verificada contra respuesta real (0 duplicados) ✅; periodos sin respuesta `200` documentados explícitamente ✅; verificación en vivo propia contra Postgres, con prueba automática ✅. | ADS-15. | P0 | M | 1 |
| LEG-02 | **IMPLEMENTADO** (API) | `GET /api/proyectos` (filtros: periodo, estado, autor, texto libre simple), `GET /api/proyectos/:periodo/:numero`, `GET /api/proyectos/periodos`. | Paginación obligatoria ✅; filtro sin match responde lista vacía, distinguible de un periodo no disponible vía `/periodos` ✅; tests con y sin match, incluido el detalle por `:periodo/:numero` ✅; verificado en vivo contra la API real levantada localmente. | LEG-01. | P0 | S | 1 |
| LEG-03 | **IMPLEMENTADO** (MCP + docs) | Registrar tool MCP, ficha en `docs/conectores.md`, data contract. | Tool sigue patrón `SIN_SCHEDULER` ✅; `scripts/check-connectors-documented.sh` y `mcp-server/src/__tests__/routes-vs-catalog.test.ts` pasan ✅; ficha en `docs/conectores.md` y `docs/data-contracts/legislativo-congreso-proyectos.md` ✅. | LEG-02. | P0 | S | 1 |
