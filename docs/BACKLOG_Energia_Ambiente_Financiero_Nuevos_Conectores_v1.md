...
| GEO-02 | **IMPLEMENTADO** (Geoespacial) | App/conector de áreas naturales protegidas (SERNANP, ArcGIS REST). Verificado en vivo 2026-09-21: 466/466 filas ingeridas (5 capas), 0 rechazadas (4 en La Libertad). Sin ID único estable (código de área se repite en geometrías multi-parte) — se implementó snapshot completo por capa, sin upsert incremental. | Respuesta real de `?f=json` y `/query` de las 5 capas en el PR ✅; ID no único documentado explícitamente y resuelto con snapshot completo ✅. | Ninguna. | P1 | M | 1 |
