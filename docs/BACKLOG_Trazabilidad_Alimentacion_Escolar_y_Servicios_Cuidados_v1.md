...
| SC-10 | Código listo; datos bloqueados | La consulta por cumplimiento solo acepta RUC exacto; aún no existe uno en los lotes materializados. **Intento de resolución 2026-09-06**: `contratacionesabiertas.osce.gob.pe` (portal OCDS vigente de OSCE) no resuelve DNS desde este entorno; el dataset "Qali Warma Proveedores" de datosabiertos.gob.pe está desactualizado a 2021 (anterior a los lotes 2025). Una búsqueda manual fuera de este entorno encontró nombres/RUC candidatos para ambos consorcios, pero sin URL de fuente primaria citable — no se cargan, por regla del proyecto (SC-05: RUC exige URL y fecha verificables). Registrado como intento en la cola de revisión, no como dato. Ver `apps/radar-ejecucion/api/src/db/migrations/020_sc10_ruc_verificacion_2026-09-06.sql`. |
