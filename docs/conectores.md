...
| **DNI hasheado, nunca en texto plano** | La fuente expone `strDocumentoIdentidad` sin enmascarar (mismo problema que ya llevó a excluir el recurso histórico de `autoridades-connector.ts`). Decisión explícita del usuario 2026-09-22: se guarda solo un hash SHA-256 del DNI (`dni_hash`) — sirve para detectar duplicados/reemplazos de la misma persona entre cargos, pero no permite reconstruir el documento original. |
