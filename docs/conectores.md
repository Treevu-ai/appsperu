...
### `autoridades-vigentes-connector.ts`

Construido 2026-09-22, a partir de un gap real encontrado en vivo: ni `autoridades-connector.ts`
(solo nacional, ver arriba) ni `candidatos-erm` (solo candidatos inscritos a la próxima elección,
sin resultado) permitían responder "¿quién es la autoridad municipal vigente de este distrito
hoy?" — bloqueó directamente un análisis del caso de la Municipalidad Distrital de Sarín ese
mismo día.

| | |
|---|---|
| **Descripción** | Autoridades **vigentes hoy** (alcaldes y regidores, distritales y provinciales) del mandato en curso — no candidatos, no proclamación histórica. Verificado en vivo 2026-09-22 contra el ubigeo 120307 (Sarín, Sánchez Carrión, La Libertad): devuelve al alcalde y regidores del período 2023-2026 (`idProcesoElectoral=113`, "ELECCIONES REGIONALES Y MUNICIPALES 2022"), con nombre, cargo, organización política, fecha de fin de mandato y enlaces públicos a hoja de vida/plan de gobierno. |
| **Fuente de datos** | API pública del propio JNE, `cej.jne.gob.pe/Autoridades` (la SPA "Consulta de Expedientes Jurisdiccionales" / "Autoridades vigentes") — no `datosabiertos.gob.pe` (ese dataset no cubre este nivel, ver decisión de alcance de `autoridades-connector.ts` arriba). Endpoint real: `POST /Autoridades/ListarConformacionActual` con `{idTipoEleccion, strUbigeo}` — encontrado inspeccionando el bundle Angular del sitio (`AutoridadesCtrl.js`), no documentado públicamente. El catálogo completo de ubigeos y sus códigos sale de `GET /Autoridades/getDatosIniciales`. Sin WAF/Incapsula bloqueando estas dos rutas específicas (a diferencia de otras páginas del mismo dominio) — confirmado con requests directos sin sesión de navegador. |
| **Cómo lo hace** | Descarga el catálogo de ubigeos, filtra a los 1,899 distritos + 196 provincias del territorio nacional (excluye circunscripciones de voto en el extranjero, departamento ≥91), y hace un `POST` secuencial por cada uno (`idTipoEleccion=6` distrital, `=5` provincial) con 250ms de espera entre llamadas. Upsert por `id_conformacion_detalle` (identificador estable que la propia fuente asigna a cada autoridad-cargo). |
| **DNI hasheado con HMAC-SHA-256 (clave separada), nunca en texto plano ni con hash directo** | La fuente expone `strDocumentoIdentidad` sin enmascarar (mismo problema que ya llevó a excluir el recurso histórico de `autoridades-connector.ts`). Decisión explícita del usuario 2026-09-22: se guarda solo un hash del DNI (`dni_hash`) — sirve para detectar duplicados/reemplazos de la misma persona entre cargos, sin guardar el documento original. **Corrección de CodeRabbit sobre la versión inicial (mismo PR):** un SHA-256 directo de un DNI peruano (8 dígitos, ~10⁸ combinaciones) es enumerable offline en segundos si alguien accede a la base — no cumplía la promesa de "no reconstruible". Se usa HMAC-SHA-256 con `DNI_HASH_SECRET` (variable de entorno, no versionada, separada de las credenciales de base de datos) en su lugar — sin esa clave, el hash no es enumerable por fuerza bruta. |
| **Frecuencia** | Manual (`npm run ingest:autoridades-vigentes` en `apps/autoridades-electas/api`). Recorrido completo toma ~9 minutos por el rate limit. |
| **Cobertura real ingerida** | Verificado en vivo 2026-09-22: 2,095 ubigeos consultados (1,899 distritos + 196 provincias), 12,644 filas insertadas, 205 ubigeos sin autoridad vigente registrada en la fuente (dato ausente declarado, no asumido como cero). |
| **Bug real encontrado y corregido en la primera corrida** | La migración inicial (002) tenía `dni_hash NOT NULL`, pero varias filas reales de la fuente no traen DNI — la ingesta se detenía en esos casos (confirmado: ubigeo 021203, HUAYLLAPAMPA). Corregido con la migración `003_dni_hash_nullable.sql` (nunca se edita una migración ya aplicada) aplicada en caliente durante la misma corrida; el único ubigeo afectado por el error se reingirió después sin problema. |
| **Reconciliación de autoridades reemplazadas/vacantes (corrección de CodeRabbit)** | La primera versión trataba `success:false` (error real de la API) igual que una respuesta vacía-pero-exitosa (ej. cargo vacante), y nunca eliminaba filas de una corrida anterior que ya no aparecían en la respuesta actual — una autoridad reemplazada quedaría marcada como "vigente" para siempre. Corregido: `success:false` ahora lanza y cuenta como error real; una respuesta vacía-pero-exitosa borra cualquier fila previa de ese ubigeo+tipo; y cada corrida exitosa con datos borra las filas cuyo `id_conformacion_detalle` ya no está en la respuesta actual. |
| **Limitación conocida** | Solo cubre nivel Distrital y Provincial — Regional (gobernadores) queda fuera de esta versión a propósito, es un universo mucho más pequeño (25) y de menor prioridad que el gap real encontrado. |
| **Cruces** | Ninguno implementado todavía — candidato natural: cruzar por `dni_hash` contra el propio `candidatos_erm` (que también podría hashear su DNI para el cruce) para ver qué autoridades vigentes también postulan en las Elecciones Regionales y Municipales 2026. |
...
| **Cruces** | `GET /api/crossref/conflicto-uso-suelo` (ticket CFM-02, `docs/BACKLOG_Conflicto_Uso_Suelo_Forestal_Minero_v1.md`): cruce territorial por distrito entre concesiones forestales vigentes y derechos mineros titulados de `catastro-minero` (`estado='T'`), traduciendo los UBIGEO de SERFOR a nombre real vía `territories` de ceplan-geo. Requiere `CEPLAN_GEO_DATABASE_URL`/`CATASTRO_MINERO_DATABASE_URL`. Sin geometría real (ninguna de las dos fuentes la trae en este conector) — solo coincidencia de distrito, no superposición de polígonos. Verificado en vivo 2026-09-21 contra MADRE DE DIOS: el distrito de Huepetuhe (zona de minería informal ampliamente documentada) tiene 190 derechos mineros titulados (32,105 ha) coexistiendo con 5 concesiones forestales vigentes (32,957 ha) — superficies casi idénticas. Spike real: `docs/spike-conflicto-uso-suelo-forestal-minero-2026-09.md`. Tool MCP `catastro_forestal_conflicto_uso_suelo`. Overlay geoespacial real (polígonos) contra `catastro-minero`/`areas-protegidas` sigue sin implementar. |
...
| **Cruces** | `GET /api/crossref/preparacion-riesgo` (ticket PRV-02, `docs/BACKLOG_Preparacion_Riesgo_Fenomeno_Nino_v1.md`): cruce territorial por distrito entre historial de emergencias tipo El Niño y proyectos de inversión de prevención (Invierte.pe, filtro por nombre, no exhaustivo), enriquecido con estado de ejecución en INFOBRAS. Requiere `INVERSIONES_DATABASE_URL`; `INFOBRAS_DATABASE_URL` es enriquecimiento adicional opcional — sin ella (o si la consulta falla), `obrasInfobras`/`obrasParalizadas` son `null`, no `0` (`infobrasEstado` en la respuesta lo indica). Verificado en vivo 2026-09-21 contra LA LIBERTAD: 9 de los 10 distritos con más emergencias no tienen proyectos de prevención detectados por este filtro (la ausencia no implica que el distrito no tenga ninguno real); una defensa ribereña del río Chicama está paralizada (64.83% de avance). Inversiones sin distrito asignable (ej. el sentinel `"- TODOS -"` de Invierte.pe para proyectos multi-distrito) se excluyen de `distritos`/`totalDistritos` y se reportan aparte en `proyectosSinDistritoAsignado`. Spike real: `docs/spike-preparacion-riesgo-nino-2026-09.md`. Tool MCP `emergencias_indeci_preparacion_riesgo`. Impacto económico de emergencias cruzado contra `bcrp-la-libertad`/`radar-ejecucion` sigue sin implementar. |
...
| `autoridades-vigentes-connector.ts` | autoridades-electas | JNE Autoridades Vigentes (cej.jne.gob.pe, API no documentada) | POST secuencial por ubigeo (1,899 distritos + 196 provincias), rate-limited, DNI hasheado | Manual | Nacional completo a nivel Distrital/Provincial (Regional fuera de alcance) |
