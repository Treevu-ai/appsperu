...
- **Evaluación (2026-09-08):** sí cambiaría significativamente el ranking. Se simularon 2 esquemas de ponderación contra las 129 entidades de La Libertad con score, comparando contra el promedio simple actual:
  - **Esquema A** (ejecución con peso doble, resto peso 1): correlación de Spearman **0.9167** contra el ranking actual — **50 de 129 entidades (39%) se mueven más de 10 puestos**, 9/10 se mantienen en el top-10. Mayor movimiento: Municipalidad Distrital de Ucuncha, puesto 62 → 105 (+43).
  - **Esquema B** (ejecución e inversiones con peso doble): Spearman **0.9293**, mismas 50 entidades movidas más de 10 puestos, 10/10 se mantienen en el top-10 pero con reordenamiento interno fuerte (ej. Corte Superior de Justicia de La Libertad, puesto 86 → 41).
  - **No hay una justificación de negocio ya decidida** para que ejecución/inversiones pesen más que compras o salud tributaria — ambos esquemas fueron hipótesis de prueba, no una propuesta con respaldo. Dado el movimiento real y material del ranking (39% de las entidades cambia de posición de forma significativa), esto **no es un ajuste cosmético** — cambia qué municipalidades aparecen arriba o abajo de la tabla, con consecuencias reputacionales reales si se publica.
  - **Cerrado como "evaluado, impacto confirmado — requiere decisión explícita del usuario sobre pesos antes de implementar"**, consistente con la regla transversal de este documento (SI-04 y SI-05 no se implementan sin esa confirmación). No se implementó ningún esquema de ponderación en esta evaluación.
...
- **Evaluación (2026-09-08):** las 3 candidatas evaluadas contra las dos preguntas del criterio de aceptación — ninguna se recomienda agregar tal cual está planteada:
  - **`causalParalizacion` (INFOBRAS):** solo aplica a obras con `existeParalizacion=true`, ~2.5% del universo de obras (252 de 10,134 en La Libertad, ver DQ-03/resumen) — como componente por entidad quedaría casi siempre en 100 (sin señal real) salvo para el puñado de entidades con obras paralizadas. Además, varias causales son externas a la gestión de la entidad (ej. "fenómenos climatológicos"/"eventos climáticos" — 3 de las 10 obras más paralizadas de Pataz citan esto, ver artefacto de esa provincia), no una característica de desempeño institucional. **No se recomienda**: mide mayormente clima/terceros, no gestión.
  - **"Modalidad de control" de Contraloría (`informes-control`):** existen 11 modalidades reales para La Libertad (verificado en vivo: Orientación de Oficio 3,090, Control Concurrente 1,612, Auditoría Financiera 263, etc.), pero la modalidad refleja qué tipo de servicio programó Contraloría, no el desempeño de la entidad auditada — es una característica del proceso de control, no de la entidad. El campo del mismo dataset que sí sería una señal de desempeño real es `esConResponsabilidad` (296 de 7,699 informes, 85 de 194 entidades distintas con al menos un hallazgo de responsabilidad) — pero **`informes-control` no tiene ningún `entity_crosswalk` hacia `radar-ejecucion`** (verificado: solo 3 tablas en su base, ninguna de crosswalk) — construirlo es trabajo real de conector nuevo, no una reutilización de dato ya cruzado como los otros 5 componentes actuales. **No se recomienda con el campo propuesto** (modalidad); `esConResponsabilidad` es una idea distinta y más prometedora para una evaluación futura aparte, no evaluada a fondo aquí porque no era la candidata original de este ticket.
  - **`tipoInfraccion` (OEFA, `infracciones-ambientales`):** hallazgo estructural — esta fuente sanciona **administrados privados** (`nombre_administrado`, empresas/personas, 610 filas / 160 administrados distintos en La Libertad), no entidades de gobierno. No hay ninguna entidad de gobierno (`entity_code` de radar-ejecucion) a la que atribuirle una infracción ambiental en este modelo de datos — OEFA es quien sanciona, no un gobierno local o regional. **No se recomienda: incompatibilidad estructural**, no es una decisión de peso/cobertura sino que el dato no describe al mismo tipo de sujeto que el resto del score.
  - **Cerrado como "evaluado, diferido — ninguna de las 3 candidatas originales se agrega".** `esConResponsabilidad` de `informes-control` queda anotado como candidata más prometedora para una futura evaluación (requiere construir un crosswalk nuevo primero).
