...
- `SIN_RUC_OFICIALMENTE_VINCULADO`: no se ha ingresado un documento que una proveedor, lote y servicio; Rastro no busca un nombre parecido en compras generales.
...
El expediente de Casa Grande incluye una causal de penalidad vinculada al expediente de conformidad de entrega. Rastro la conserva como `OBSERVACION_CONTRACTUAL_DOCUMENTADA`: no la convierte en prueba de que un alimento no llegó a un colegio ni en una conclusión sobre el proveedor.
...
Las observaciones no cambian el estado de un lote ni de una entrega. Rastro conserva cinco tipos: `SANCION_FORMAL`, `DENUNCIA_CON_EXPEDIENTE`, `PROCESO_EN_CURSO`, `ANTIGUEDAD_RUC` y `REFERENCIA_EXTERNA`.
