...
| `renamu` / `renamu_municipalidades` | `(idmunici, anio)` | 2024-2025 (2 años) | **Panel sin filtro por defecto — candidato a revisar** | `GET /api/municipalidades` sin filtro devuelve ambos años mezclados (ej. Pataz: 26 filas para 13 municipalidades, no 13). El endpoint hermano `GET /api/equipamiento` sí filtra a `ORDER BY anio DESC LIMIT 1` por ubigeo — inconsistencia entre los dos endpoints de la misma app. No es tan grave como los dos casos de arriba porque no hay un total numérico que se infle (es un registro de identidad, no un conteo agregado), pero un consumidor que haga `.length` para contar municipalidades se equivoca 2x. Fuera de alcance de DQ-03/DQ-04 (no estaba en el hallazgo original) — candidato a ticket de seguimiento si se prioriza. |
