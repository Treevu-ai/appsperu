...
| `renamu` / `renamu_municipalidades` | `(idmunici, anio)` | 2024-2025 (2 años) | **Panel sin filtro por defecto — ticket abierto (DQ-16)** | `GET /api/municipalidades` sin filtro devuelve ambos años mezclados (ej. Pataz: 26 filas para 13 municipalidades, no 13). El endpoint hermano `GET /api/equipamiento` sí filtra a `ORDER BY anio DESC LIMIT 1` por ubigeo — inconsistencia entre los dos endpoints de la misma app. No es tan grave como los dos casos de arriba porque no hay un total numérico que se infle (es un registro de identidad, no un conteo agregado), pero un consumidor que haga `.length` para contar municipalidades se equivoca 2x. Ver [DQ-16](../TICKETS_Calidad_Datos_Auditoria_La_Libertad_v1.md#dq-16--corte-vigente-por-defecto-en-renamumunicipalidades--colapso-de-anio_fiscal-en-radar-ejecucion). |
...
| `radar-ejecucion` / `budget_execution` | `(entity_code, funcion, anio_fiscal, fecha_corte[, meta_departamento])` | Por año fiscal — solo 2026 ingerido en este entorno de desarrollo | **Riesgo latente, mismo patrón que DQ-03/DQ-04 — ticket abierto (DQ-16), no verificable hoy por falta de datos multi-año** | `LATEST_BUDGET_CTE` (`packages/shared-queries`) dedupe por `(entity_code, funcion, anio_fiscal, meta_departamento, generica)` — `anio_fiscal` es parte de la clave de dedupe, así que **no colapsa entre años fiscales distintos**. Si se ingiere más de un año fiscal, `GET /api/execution` sin `anio` devolvería todos los años mezclados, igual que pasaba con `infraestructura-mtc` antes de DQ-03. Hoy el entorno de desarrollo solo tiene 2026 ingerido, así que el riesgo no se manifiesta todavía — pero no hay ningún filtro de "año vigente por defecto" que lo prevenga si se ingiere un segundo año. Ver [DQ-16](../TICKETS_Calidad_Datos_Auditoria_La_Libertad_v1.md#dq-16--corte-vigente-por-defecto-en-renamumunicipalidades--colapso-de-anio_fiscal-en-radar-ejecucion) — revisar antes de ingerir un año fiscal adicional. |
